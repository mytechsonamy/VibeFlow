...
### Thirteen test layers — run all thirteen before declaring a sprint ticket done
...
- **Hook scripts (bash):** `bash hooks/tests/run.sh` — 75 assertions covering every hook + shared `_lib.sh` + S4-02 hardening + S4-08 output budgets + S11-C filesystem-first helpers + S13-A phase-write-guard decisions
