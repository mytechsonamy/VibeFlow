...
### Seventeen test layers — run all seventeen before declaring a sprint ticket done
...
- **Hook scripts (bash):** `bash hooks/tests/run.sh` — 110 assertions covering every hook + shared `_lib.sh` + S4-02 hardening + S4-08 output budgets + S11-C filesystem-first helpers + S13-A phase-write-guard decisions + S14-A/B consensus CLI-gating + non-reviewer filtering + S16-A consensus-gate marker/pass-through/bypass + S17-A round-aware aggregator + S17-B critical dedup + Jaccard title similarity + S17-C load-sdlc-context override advisory
