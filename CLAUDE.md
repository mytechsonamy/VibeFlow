...
- **Unit (vitest) — 5 MCP servers, each with 80/80/80/80 coverage threshold:**
  - `cd mcp-servers/sdlc-engine && npm test` — 142 tests (engine, consensus, validation, phases, filesystem state store, tools, server dispatch, Bug #13 getOrInit fast-path, filesystem race tests incl. 5-process cross-process lock + 50-concurrent-writer Bug #6 regression, event model round-trips, Sprint 15-C phase-scoped consensus validation)
...
- **Hook scripts (bash):** `bash hooks/tests/run.sh` — 86 assertions covering every hook + shared `_lib.sh` + S4-02 hardening + S4-08 output budgets + S11-C filesystem-first helpers + S13-A phase-write-guard decisions + S14-A/B consensus CLI-gating + non-reviewer filtering + S16-A consensus-gate marker/pass-through/bypass
