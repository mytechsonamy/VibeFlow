...
- **Integration (bash + node):**
  ...
  - `bash tests/integration/sprint-6.sh` — 17 assertions in normal dev (21 when docker+pg available): concurrent-advance CAS stress test on real PostgreSQL [S6-A] + Next.js demo "use client" surface + optional `next build` gate [S6-B]
- Total baseline: **1469 passing checks** across **12 test layers** (1473 in live mode). Sprint 4 ✅ COMPLETE + v1.0.0 shipped. Sprint 5 ✅ COMPLETE + v1.0.1 shipped. Sprint 6 in progress (S6-01 + S6-04 + S6-07 ✅, S6-02 + S6-03 + S6-05 + S6-06 + S6-08 + S6-09 pending).
