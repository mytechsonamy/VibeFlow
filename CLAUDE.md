...
- **Integration (bash + node):**
  ...
  - `bash tests/integration/sprint-7.sh` — 26 assertions: release.sh pg peer-dep sanity check [S7-A] + RELEASING.md troubleshooting + sha256-drift recovery [S7-B] + reproducible package-plugin.sh tarball [S7-C] + sprint-7.sh harness self-audit [S7-Z]
- Total baseline: **1518 passing checks** across **13 test layers** (1522 in live mode). Sprint 4 ✅ COMPLETE + v1.0.0 shipped. Sprint 5 ✅ COMPLETE + v1.0.1 shipped. Sprint 6 ✅ COMPLETE + **v1.1.0 shipped 2026-04-16**. Sprint 7 in progress (S7-04 + S7-05 + S7-05B ✅, S7-01/02/03/06/07 pending).
