...
- **Integration (bash + node):**
  ...
  - `bash tests/integration/sprint-8.sh` — 12 assertions: sprint-7.sh [S7-C] multi-tarball save/restore fix [S8-A] + sprint-8.sh harness self-audit [S8-Z]. Includes a runtime fixture test that seeds two fake tarballs and verifies both survive a [S7-C] run.
- Total baseline: **1578 passing checks** across **14 test layers** (1582 in live mode, 1594 with `VF_RUN_PG_MATRIX=1`). Sprint 4 ✅ COMPLETE + v1.0.0 shipped. Sprint 5 ✅ COMPLETE + v1.0.1 shipped. Sprint 6 ✅ COMPLETE + v1.1.0 shipped. Sprint 7 ✅ COMPLETE + v1.2.0 shipped 2026-04-16. Sprint 8 in progress (S8-02 ✅, S8-01/03/04/05/06/07/08 pending).
