...
### Sixteen test layers — run all sixteen before declaring a sprint ticket done
- **Unit (vitest) — 5 MCP servers, each with 80/80/80/80 coverage threshold:**
  - `cd mcp-servers/sdlc-engine && npm test` — 114 tests (engine, consensus, validation, phases, state store, tools, server dispatch, Bug #13 getOrInit fast-path, S10-01 PgBouncer transaction-mode startup probe — 9 tests)
...
- **Integration (bash + node):**
  ...
  - `bash tests/integration/sprint-4.sh` — 367 assertions (coverage + io-standard + demo-app + user docs + plugin manifest + ci_provider wiring + packaging + tarball + CHANGELOG sync + offline/large-input/budget sentinels + fresh-install end-to-end simulation [S4-K] + S7-01 gitlab_token/gitlab_base_url userConfig keys + S9-07 version-matched tarball lookup)
  - `bash tests/integration/sprint-5.sh` — 98 assertions (GitLab CI provider wiring [S5-A] + live PostgreSQL team-mode walk [S5-B] + release script + workflow + CHANGELOG runtime sentinel [S5-C] — preflight loop now expects sprint-9 + sprint-10 + Next.js demo layout + artifacts [S5-D] + Bug #13 cross-process reproducer mirror [S5-E])
  ...
  - `bash tests/integration/sprint-10.sh` — 45 assertions: PgBouncer transaction-mode startup probe + TEAM-MODE.md cross-references [S10-A] + scheduled pg-matrix weekly CI workflow [S10-C] + release.sh --notes-file pre-fill (static + 4-quadrant runtime) [S10-D] + sprint-10.sh harness self-audit [S10-Z]. There is no [S10-B] — that letter is reserved for a future S10-02 (managed cloud Postgres) reopen. Runtime opt-outs: `VF_SKIP_S10A_RUNTIME=1` / `VF_SKIP_S10D_RUNTIME=1`.
- Total baseline: **1694 passing checks** across **16 test layers** (1698 in live mode, 1710 with `VF_RUN_PG_MATRIX=1`). Sprint 4 ✅ COMPLETE + v1.0.0 shipped. Sprint 5 ✅ COMPLETE + v1.0.1 shipped. Sprint 6 ✅ COMPLETE + v1.1.0 shipped. Sprint 7 ✅ COMPLETE + v1.2.0 shipped 2026-04-16. Sprint 8 ✅ COMPLETE + v1.3.0 shipped 2026-04-17. Sprint 9 ✅ COMPLETE + v1.4.0 shipped 2026-04-17. Sprint 10 ✅ COMPLETE + v1.5.0 shipped 2026-04-19. S10-01 (PgBouncer probe) + S10-03 (scheduled pg-matrix weekly CI) + S10-04 (release.sh --notes-file) shipped; S10-02 DROPPED (managed cloud Postgres — no deployment target).
