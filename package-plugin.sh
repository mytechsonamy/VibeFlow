...
# Sprint 7 / S7-05B — reproducible tarball.
#
# The previous `tar -cz -T list -f archive` invocation inherited two
# sources of non-determinism:
#   1. Every file's mtime ends up in the tar header, so consecutive
#      runs against the same tree produce slightly different bytes.
#   2. `tar -cz` uses tar's built-in gzip, which writes a gzip header
#      containing the filename + original timestamp.
# The S6-09 v1.1.0 release hit a real consequence: the sha256 sidecar
# generated by `bin/release.sh [6]` did not match the tarball that
# ended up on GitHub Releases because `sprint-4.sh [S4-G]` ran
# `package-plugin.sh` a second time during preflight, producing a
# different archive.
#
# Determinism strategy (portable across BSD tar on macOS and GNU tar
# on Linux):
#   a. Stage all whitelisted files into a fresh tempdir so we can
#      normalize without touching the working tree.
#   b. `touch -t 197001010000.00` every staged file to epoch 0.
#   c. Sort the file list — `find` output is filesystem-order, which
#      varies across runs. tar processes `-T` entries in the order
#      they appear in the file.
#   d. Detect tar variant (bsdtar on macOS → --uid=0 --gid=0; GNU tar
#      on Linux → --owner=0 --group=0) so ownership columns in the
#      tar header are normalized.
#   e. Pipe uncompressed tar to `gzip -n`, which strips the filename
#      + timestamp from the gzip header.
# Two consecutive runs of this block against the same input tree
# now produce byte-identical archives (and identical sha256).
STAGING="$(mktemp -d "${TMPDIR:-/tmp}/vf-pkg-stage.XXXXXX")"
SORTED_LIST="$TMPLIST.sorted"
# Extend the existing EXIT trap so the staging dir + sorted list are
# cleaned up on any exit path.
trap 'rm -rf "$TMPLIST" "$SORTED_LIST" "$STAGING"' EXIT

sort "$TMPLIST" > "$SORTED_LIST"
...
# Normalize mtimes on the staged copies. epoch 0 (1970-01-01 UTC) is
