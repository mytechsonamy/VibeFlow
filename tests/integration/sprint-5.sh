...
# ---------------------------------------------------------------------------
# [S5-B] was a live-PostgreSQL team-mode walk. Sprint 11-E removed the
# Postgres backend entirely — state is filesystem-only now. The cross-
# process read-after-write coverage this block guarded is preserved in
# tests/filesystem-race.test.ts (5 subprocesses × 5 writes) and in
# [S5-E] below (single-process Bug #13 reproducer via MCP).
# ---------------------------------------------------------------------------
pass "[S5-B] obsolete — filesystem backend coverage lives in filesystem-race.test.ts + [S5-E]"
