import { PhaseId, PhaseRegistry, CONSENSUS_CRITERION_PATTERN } from "./phases.js";
...
export class PhaseTransitionValidator {
  constructor(private readonly registry: PhaseRegistry) {}
  ...
  validate(req: PhaseTransitionRequest): TransitionResult {
    ...
    if (toIdx < fromIdx) {
      errors.push(
        `Backward transition not permitted: ${req.from}→${req.to}`,
      );
    }

    if (toIdx !== fromIdx + 1) {
      errors.push(
        `Invalid transition ${req.from}→${req.to}: ` +
    ...
    for (const criterion of current.exitCriteria) {
      ...
      if (scopeMatch) {
        const expectedPhase = scopeMatch[1]!.toUpperCase() as PhaseId;
        const lastPhaseOk =
    ...
    const hasScopedConsensus = current.exitCriteria.some((c) =>
      CONSENSUS_CRITERION_PATTERN.test(c),
    );
    if (
      !hasScopedConsensus &&
      ...
      errors.push(
        `Cannot advance from ${req.from}: last consensus is ${req.lastConsensus}, expected ${ConsensusStatus.APPROVED}`,
      );
    }
