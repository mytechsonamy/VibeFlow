...
export class PostgresStateStore implements StateStore {
   ...
   */
  private async probePoolerMode(): Promise<void> {
    if (process.env.VF_SKIP_POOLER_CHECK === "1") return;
    ...
    try {
      ...
      throw new Error(
        `PgBouncer probe failed during pool.connect(): ${(err as Error).message}`,
      );
    }
    ...
    try {
      ...
      try {
        ...
        process.stderr.write(
          `[sdlc-engine] pg client.release() failed during PgBouncer probe: ${
 ...
 */
async function readBackendPid(client: PgClientLike): Promise<number> {
  await client.query("BEGIN");
  try {
    await client.query("SHOW search_path");
    const res = await client.query<{ pid: number | string }>(
      "SELECT pg_backend_pid() AS pid",
    );
    const pid = Number(res.rows[0]?.pid);
    if (!Number.isFinite(pid) || pid <= 0) {
      throw new Error(
        `PgBouncer probe could not read pg_backend_pid (got '${
          res.rows[0]?.pid
        }')`,
      );
    }
