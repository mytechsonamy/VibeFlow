...
import { EVENT_SCHEMA_VERSION, deriveEvent, eventFileStem, parseEventJson, parseEventMd, renderEventMd, serializeEventJson, } from "./events.js";
...
export class FilesystemStateStore {
    ...
    async read(projectId) {
        ...
        try {
            ...
            if (parsed.schemaVersion !== ROLLUP_SCHEMA_VERSION) {
                throw new Error(`Rollup schemaVersion ${parsed.schemaVersion} not supported ` +
    ...
    async archiveCompletedPhase(projectId, phase, _lastRevision) {
        const archiveBucket = path.join(this.archiveDir(projectId), `${bucketForPhase(phase)}-${phase}`);
    ...
    async rebuildMissingMdTwins(projectId) {
        ...
        for (const jsonName of jsonNames) {
            ...
            if (!mdNames.has(mdTwin)) {
                try {
                    const jsonBody = await fsp.readFile(path.join(eventsDir, jsonName), "utf8");
                    const event = parseEventJson(jsonBody);
                    await fsp.writeFile(path.join(eventsDir, mdTwin), renderEventMd(event));
                }
                catch {
        ...
        for (const mdName of mdNames) {
            ...
            if (!jsonNames.has(jsonTwin)) {
                try {
                    const mdBody = await fsp.readFile(path.join(eventsDir, mdName), "utf8");
                    const event = parseEventMd(mdBody);
                    await fsp.writeFile(path.join(eventsDir, jsonTwin), serializeEventJson(event));
                }
                catch {
                    // Skip unparseable.
                }
