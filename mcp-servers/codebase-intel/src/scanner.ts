...
const FRAMEWORK_PACKAGES: ReadonlyMap<string, string> = new Map([
  ["fastify", "fastify"],
  ["express", "express"],
  ["@nestjs/core", "nestjs"],
  ["next", "next.js"],
...
const TEST_RUNNER_PACKAGES: ReadonlyMap<string, string> = new Map([
  ["vitest", "vitest"],
  ["jest", "jest"],
  ["mocha", "mocha"],
  ["ava", "ava"],
...
const BUILD_TOOL_PACKAGES: ReadonlyMap<string, string> = new Map([
  ["vite", "vite"],
  ["webpack", "webpack"],
  ["rollup", "rollup"],
  ["esbuild", "esbuild"],
...
class FindingSet {
  private readonly map = new Map<string, { evidence: Set<string>; confidence: number }>();

  add(name: string, evidence: readonly string[], confidence: number): void {
  ...
  upgrade(name: string, evidence: readonly string[], confidence: number): void {
...
function detectLanguagesFromExt(
  ...
  if (ts > 0) out.push(["typescript", ts]);
  if (js > 0 && ts === 0) out.push(["javascript", js]);
  if (py > 0) out.push(["python", py]);
  if (go > 0) out.push(["go", go]);
  if (rs > 0) out.push(["rust", rs]);
  return out;
}
