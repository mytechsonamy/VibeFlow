...
export interface CiProvider {
  ...
  triggerWorkflow(input: CiTriggerInput): Promise<{ accepted: boolean; note: string }>;
...
export function createGithubClient(opts: GithubClientOptions): CiProvider {
  const token = opts.token ?? process.env.GITHUB_TOKEN ?? "";
  if (!token) {
    throw new CiConfigError(
      "GITHUB_TOKEN is required. Set it via plugin userConfig " +
  ...
  async function request<T>(
    ...
    try {
      ...
      throw new CiClientError(
        `github request failed (transport): ${(err as Error).message}`,
        { status: 0, path },
      );
    }
    if (!res.ok) {
      ...
      throw new CiClientError(
        `github ${res.status} ${res.statusText} on ${path}: ${snippet}`,
    ...
    try {
      ...
      throw new CiClientError(
        `github response was not valid JSON on ${path}: ${body.slice(0, 120)}`,
        { status: res.status, path },
      );
    }
  }
  ...
  return {
    ...
    async triggerWorkflow(input) {
      ...
      await request(
        `/repos/${encodeURIComponent(owner)}/${encodeURIComponent(repo)}/actions/workflows/${encodeURIComponent(input.workflow)}/dispatches`,
        {
          method: "POST",
          body: {
            ref: input.ref,
    ...
    async getRun(runId) {
      const raw = await request<GithubRunPayload>(
    ...
    async listArtifacts(runId) {
      const raw = await request<{ artifacts?: readonly GithubArtifactPayload[] }>(
        `/repos/${encodeURIComponent(owner)}/${encodeURIComponent(repo)}/actions/runs/${encodeURIComponent(runId)}/artifacts`,
      );
...
function normalizeGithubRun(raw: GithubRunPayload): PipelineRun {
  return {
    id: String(raw.id),
...
function normalizeGithubArtifact(raw: GithubArtifactPayload): PipelineArtifact {
  return {
    id: String(raw.id),
    name: raw.name,
...
function normalizeStatus(s: string): PipelineRun["status"] {
  if (s === "queued" || s === "in_progress" || s === "completed") return s;
  // GitHub occasionally uses "requested", "waiting", "pending" — all map to queued.
  if (s === "requested" || s === "waiting" || s === "pending") return "queued";
  return "in_progress";
}

function normalizeConclusion(c: string | null): PipelineRun["conclusion"] {
...
function defaultFetch(): FetchImpl {
  ...
  if (typeof g.fetch !== "function") {
    throw new CiConfigError(
      "globalThis.fetch is not available. Node 18+ is required, or pass a fetchImpl.",
    );
  }
  return async (input, init) => {
    const r = (await g.fetch!(input, init as unknown)) as {
      ok: boolean;
      status: number;
      statusText: string;
      text: () => Promise<string>;
