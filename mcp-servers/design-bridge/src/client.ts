...
export class FigmaClient {
  ...
  constructor(opts: FigmaClientOptions = {}) {
    ...
    if (!token) {
      throw new FigmaConfigError(
        "FIGMA_TOKEN is required. Set it via plugin userConfig " +
   ...
   */
  async getNodes(fileKey: string, nodeIds: readonly string[]): Promise<unknown> {
    if (!fileKey) {
  ...
  private async get(path: string): Promise<unknown> {
    ...
    try {
      ...
      throw new FigmaClientError(
        `figma request failed (transport): ${(err as Error).message}`,
        { status: 0, path },
      );
    }
    ...
    if (!res.ok) {
      ...
      throw new FigmaClientError(
        `figma ${res.status} ${res.statusText} on ${path}: ${snippet}`,
    ...
    try {
      ...
      throw new FigmaClientError(
        `figma response was not valid JSON on ${path}: ${body.slice(0, 120)}`,
        { status: res.status, path },
      );
    }
  }
